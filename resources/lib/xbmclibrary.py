...
def _export_episode(data, season_dir, makeNFO=True):
    filename = 'S{0:02d}E{1:02d} - {2}'.format(data['season_num'], str(data['order_rank'])[-2:], _clean_filename(data['title']))
    ...
    if added_folders:
        response = xbmcgui.Dialog()\
            .yesno("Added {0} Folders to Video Sources".format(db_common.SERVICE_NAME),
                   ...
                   "2. After restarting, you must configure the content type of the {0} folders in the File section".format(db_common.SERVICE_NAME),
